...
export function insertService(data, options = {}) {
  ...
  db.prepare(
    `INSERT INTO services (
      ...
      image_url, image_blob, out_of_stock, offer_type, offer_expires_at, sort_order, updated_at
    ) VALUES (
      @id, @icon, @accent, @typeEn, @typeAr, @nameEn, @nameAr,
      @descriptionEn, @descriptionAr, @priceMonth, @priceYear,
      @imageUrl, @imageBlob, @outOfStock, @offerType, @offerExpiresAt, @sortOrder, datetime('now')
    )`,
  ).run({
    id: data.id,
    icon: data.icon || "✨",
